...
def load_model(model, checkpoint, args, mode='exact', train_mode='finetune', verbose=True, DEBUG=False):
    ...
    if checkpoint in [None, 'None']:
        ...
        # if we need to initialize the model, then all weights need to be initialized.
        logger.info(f'number of keep_keys={len(keep_keys)}, number of model_keys={len(model.state_dict().keys())}')
        if len(keep_keys) != len(model.state_dict().keys()):
            raise ValueError("if we need to initialize the model, then all weights need to be initialized.")
