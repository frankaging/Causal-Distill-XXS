...
def main():
    ...
    os.environ["WANDB_PROJECT"] = "DynaSent-Distill"
    ...
    assert data_args.task_name in ["dynasent",]
    ...
    if data_args.dataset_name is not None and os.path.isdir(data_args.dataset_name):
        ...
        raise ValueError("Need a huggingface datasets formatted directory for `dataset_name`.")
    # See more about loading any type of standard or custom dataset at
    # https://huggingface.co/docs/datasets/loading_datasets.html.

    # Labels
    label_list = [0, 1, 2] # Ours: this is hard-coded now.
    ...
    if data_args.task_name == "dynasent":
        metric = load_metric("accuracy")
    else:
        metric = load_metric("accuracy") # Ours: maybe something else for ABSA?
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.task_name in ["dynasent",]:
            result = metric.compute(predictions=preds, references=p.label_ids)
            if len(result) > 1:
                result["combined_score"] = np.mean(list(result.values())).item()
                result["Macro-F1"] = result_to_print["macro avg"]["f1-score"] # Ours: this is a better metrics.
            return result
        else:
            assert False
